...
go test ./... -count=1                           # or: task test
go vet ./...
golangci-lint run --fix ./... --timeout=5m       # or: task lint
go run ./cmd/claudebox run -w . -p "..."         # run the CLI (never build-then-exec)
```
...
assets/                embedded Dockerfile + entrypoint.sh (go:embed)
```

See `docs/ARCHITECTURE.md` (diagrams), `docs/DESIGN.md`, `docs/adr/`.

## Invariants

- **Docker SDK module:** import paths `docker/docker/{client,api/types,pkg/*}` come
  from the monolithic module `github.com/docker/docker@vXX+incompatible` — NOT
